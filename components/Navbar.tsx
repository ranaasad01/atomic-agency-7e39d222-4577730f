        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          >
            <span className="w-7 h-7 rounded-sm bg-amber-500 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-200">
              <span className="text-black font-black text-xs tracking-tighter">TS</span>
            </span>
          ...
          {/* Mobile menu button */}
          <button
            className="md:hidden p-2 text-white/60 hover:text-white transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 rounded-sm"
            ...
            <nav className="max-w-7xl mx-auto px-6 py-4 flex flex-col gap-1">
              {navLinks.map((link) => (
                <Link
                  ...
                  href={getHref(link.href)}
                  onClick={(e) => handleAnchorClick(e, link.href)}
                  className={`px-3 py-2.5 text-sm font-medium rounded-sm transition-colors duration-200 ${
                    isActive(link.href)
              ...
              <Link
                href={getHref(CTA_HREF)}
                onClick={(e) => handleAnchorClick(e, CTA_HREF)}
                className="mt-2 px-3 py-2.5 bg-amber-500 text-black text-sm font-bold rounded-sm hover:bg-amber-400 transition-colors duration-200 text-center"
              >
                {CTA_LABEL}
              </Link>
            </nav>
